  ...
  //@@ Do not use shared memory to write this kernel
    ...
    if ((row < numARows) && (column < numBColumns))
    {
        ...
        for (int i = 0; i < numAColumns; ++i)
            temp += A[row * numAColumns + i] * B[column + i * numBColumns];
        C[row * numBColumns + column] = temp;
    }
}
...
int main(int argc, char **argv)
{
  ...
  matrixMultiply <<<grid, block >> > (deviceA, deviceB, deviceC, numARows, numAColumns, numBColumns);
